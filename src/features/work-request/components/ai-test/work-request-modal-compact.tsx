...
export function WorkRequestModalCompact({
  workRequest,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button
          variant="outline"
          className="bg-gradient-to-r from-green-500 to-teal-500 text-white border-none text-sm"
        >
                  ...
                  <Card>
                    ...
                    <CardContent className="text-sm">
                      <p>
                        ...
                        {format(workRequest.createdAt, 'PPpp')}
                      </p>
                      <p>
                        <span className="font-medium">Area:</span>{' '}
                        {workRequest.area.name}
                      </p>
                      <p>
                      ...
                      {workRequest.runningHour && (
                        <p>
                          <span className="font-medium">Running Hour:</span>{' '}
                          {workRequest.runningHour}
                        </p>
                      )}
                    </CardContent>
...
"
                    ...
                    <CardContent className="space-y-2">
                      <EmployeeInfo
                        title="Creator"
                        employee={workRequest.creator}
                      />
                      {workRequest.maintEngr && (
                        <EmployeeInfo
                          title="Maintenance Engineer"
                          employee={workRequest.maintEngr}
                        />
                      )}
                    </CardContent>
                  </Card>
                    ...
                    <CardContent className="text-sm space-y-1">
                      ...
                      {workRequest.workStartedAt && (
                        <p>
                          ...
                          {format(workRequest.workStartedAt, 'PPpp')}
                        </p>
                      )}
                      {workRequest.workFinishConfrimedAt && (
                        <p>
                          <span className="font-medium">Work Finished:</span>{' '}
                          {format(workRequest.workFinishConfrimedAt, 'PPpp')}
                        </p>
                      )}
                    </CardContent>
                  </Card>
                  <Card>
                    ...
                    <CardContent className="text-sm">
                      ...
                      {workRequest.referredFromId && (
                        <p>
                          <span className="font-medium">Referred From:</span>{' '}
                          {workRequest.referredFromId}
                        </p>
                      )}
...
function EmployeeInfo({
  ...
  title: string;
  employee: WorkRequest['creator'] | WorkRequest['maintEngr'];
}) {
  return (
